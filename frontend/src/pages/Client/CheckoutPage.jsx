...
export default function CheckoutPage() {
  ...
  const isEligibleAmount = cartTotal >= 300000;
  const missingAmount = 300000 - cartTotal;
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isEligibleAmount) {
      setErrorMsg(`Đơn hàng của bạn đạt ${cartTotal.toLocaleString('vi-VN')}đ. Tiệm chỉ nhận đơn từ 300.000đ trở lên.`);
      return;
    }
      ...
      <div style={{ maxWidth: '1120px', margin: '0 auto' }}>
        ...
        <div style={{
          ...
          >
            ...
            <button
              type="submit"
              disabled={isSubmitting || !isEligibleAmount || cartItems.length === 0}
              style={{
                backgroundColor: isEligibleAmount && cartItems.length > 0 ? '#451A03' : '#A89990',
                color: '#FFFFFF',
                border: 'none',
                borderRadius: '12px',
                padding: '1rem',
                fontSize: '1.05rem',
                fontWeight: '700',
                cursor: isEligibleAmount && cartItems.length > 0 && !isSubmitting ? 'pointer' : 'not-allowed',
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
                gap: '0.5rem',
                boxShadow: isEligibleAmount ? '0 6px 18px rgba(69, 26, 3, 0.25)' : 'none',
                transition: 'all 0.2s'
              }}
            >
              {isSubmitting ? (
          ...
          <div style={{
            ...
            {/* Cảnh báo đơn hàng tối thiểu 300.000đ */}
            {!isEligibleAmount && (
              <div style={{
                marginTop: '1.25rem',
                ...
                <div style={{ fontWeight: '700', marginBottom: '4px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                  <AlertCircle size={16} />
                  <span>Đơn hàng chưa đạt mức tối thiểu:</span>
                </div>
                <div>
                  Tiệm Bánh Của Vy quy định đơn đặt bánh từ <strong>300.000đ</strong> trở lên. Bạn cần chọn thêm{' '}
                  <strong>{missingAmount.toLocaleString('vi-VN')}đ</strong> nữa nhé!
                </div>
                <div style={{ marginTop: '8px' }}>
                  ...
                  >
                    › Khám phá thêm các loại bánh khác
                  </Link>
                </div>
              </div>
            )}
